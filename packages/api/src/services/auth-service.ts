...
import {validateEmail, validatePassword} from "../utils/password-validation.js";
...
export class AuthService {
     ...
     */
    async register(email: string, password: string, ip: string): Promise<{message: string}> {
     ...
     */
    async forgotPassword(email: string, id: string): Promise<{message: string}> {
        const genericResponse = { message: "If the email exists, a reset link has been sent." };

        // Rate limit by email
        const rateLimit = await this.rateLimitService.checkLimit("forgot_password", email.toLowerCase());
        if (!rateLimit.allowed) return genericResponse;

        await this.rateLimitService.recordAttempt("forgot_password", email.toLowerCase());
    ...
    async resendVerification(email: string, ip: string): Promise<{message: string}> {
        const genericResponse = { message: "If the email exists and is unverified, a new verification link has been sent." };
    ...
    private async checkRateLimit(action: string, identifier: string): Promise<void> {
        const rateLimit = await this.rateLimitService.checkLimit(action, identifier);

        if (!rateLimit.allowed) {
            throw new AuthError(`Too many attempts. Try again in ${rateLimit.retryAfter} seconds.`, 429);
        }
