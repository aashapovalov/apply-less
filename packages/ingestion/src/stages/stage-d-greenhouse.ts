...
export async function runStageD(
  ...
  const { dryRun = false, companySlug = undefined } = options;
  ...
  try {
    ...
    for (const company of companies.rows) {
      try {
        ...
        if (slug === "embed") {
          console.log(
            `  ⚠️  Invalid slug 'embed' - re-run detection for this company`,
          );
        ...
        if (!slug && company.careers_page_url) {
          slug =
            greenhouseClient.extractSlugFromUrl(company.careers_page_url) ??
            undefined;
        }
        ...
        for (const ghJob of greenhouseJobs) {
          try {
            if (dryRun) {
              console.log(
                `    [DRY RUN] Would insert: ${ghJob.title} - ${ghJob.location?.name}`,
              );
              continue;
            }

            // Fetch full job details (including description)
            const jobDetail = await greenhouseClient.fetchJobDetail(
              slug!,
              ghJob.id,
            );
            ...
            if (!jobDetail) {
              console.log(
                `    ⚠️  Could not fetch details for: ${ghJob.title}`,
              );
              continue;
            }
            ...
            // Extract department
            const department =
              jobDetail.departments && jobDetail.departments.length > 0
                ? jobDetail.departments[0].name
                : undefined;

            // Check if Israeli location
            const locationResult = normalizeLocation(jobDetail.location?.name);

            if (!locationResult.isIsraeli) {
              console.log(
                `    ⏭️  Skipped (non-Israeli): ${ghJob.title} - ${jobDetail.location?.name} [${locationResult.country}]`,
              );
              stats.skippedRecords++;
              continue;
            }
            ...
            console.error(
              `    ❌ Failed: ${ghJob.title} - ${jobError.message}`,
            );
          }
        }
