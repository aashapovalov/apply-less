...
import { EmbedResponse, EmbedSingleResponse } from "../types/index.js";
...
export class EmbeddingClient {
     ...
     */
    async embedText(text: string, textType: "passage" | "query" = "passage"): Promise<number[]> {
        try {
            const response = await this.httpClient.post<EmbedSingleResponse>("/api/embed/single", {
                text,
                normalize: true,
                text_type: textType,
            });
    ...
    async embedBatch(texts: string[]): Promise<number[][]> {
       try {
           const response = await this.httpClient.post<EmbedResponse>("/api/embed/", {
               texts,
               normalize: true,
               text_type: "passage",
           });
           console.log(`  📊 ${response.data.count} embeddings in ${response.data.time_ms}ms`);
