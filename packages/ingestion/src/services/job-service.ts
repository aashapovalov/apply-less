...
export class JobService {
    ...
    async getJobsStats(): Promise<{
        ...
        const totalResult = await this.db.query(`SELECT COUNT(*) as count FROM jobs`);
        const total = parseInt(totalResult.rows[0].count);

        const activeResult = await this.db.query(`SELECT COUNT(*) as count FROM jobs WHERE status = "active"`);
